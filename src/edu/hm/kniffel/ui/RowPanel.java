...
public class RowPanel extends JPanel {
    ...
    private static final int MAX_NUMBER_LABEL_SIZE = 30;
    private static final int MAX_TEXT_LABEL_SIZE = 100;
     ...
     */
    public RowPanel(ScoringCategory category, PlayerController playerController, ScoreCardPanel scoreCardPanel) {
        this.category = category;
        this.setLayout(new BoxLayout(this, BoxLayout.LINE_AXIS));

        categoryLabel = new JLabel(" " + category.name()); // better use alignment; no time for that now
        categoryLabel.setMaximumSize(new Dimension(MAX_TEXT_LABEL_SIZE, SingleDicePanel.DICE_SIZE));
        categoryLabel.setPreferredSize(new Dimension(MAX_TEXT_LABEL_SIZE, SingleDicePanel.DICE_SIZE));
        categoryLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        this.add(categoryLabel);

        storedScoreLabel.setMaximumSize(new Dimension(MAX_NUMBER_LABEL_SIZE, SingleDicePanel.DICE_SIZE));
        storedScoreLabel.setPreferredSize(new Dimension(MAX_NUMBER_LABEL_SIZE, SingleDicePanel.DICE_SIZE));
        storedScoreLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        this.add(storedScoreLabel);

        previewScoreLabel.setMaximumSize(new Dimension(MAX_NUMBER_LABEL_SIZE, SingleDicePanel.DICE_SIZE));
        previewScoreLabel.setPreferredSize(new Dimension(MAX_NUMBER_LABEL_SIZE, SingleDicePanel.DICE_SIZE));
